...
public class TodoController {
    ...
    /** Lab3 parte 3b: PUT /todo/{listId}/tasks/{taskId}/complete */
    public void completeTodoItem(Context ctx) {
        updateCompletion(ctx, true);
    }

    /** Lab3 parte 3b: PUT /todo/{listId}/tasks/{taskId}/incomplete */
    public void reopenTodoItem(Context ctx) {
        updateCompletion(ctx, false);
    }

    private void updateCompletion(Context ctx, boolean completed) {
        int todoId = Integer.parseInt(ctx.pathParam("taskId"));
        int listId = Integer.parseInt(ctx.pathParam("listId"));
        if (!checkOwnershipOfList(ctx, listId)) {
            return;
        }
        Todo todo = todoService.setCompleted(todoId, completed);
        if (todo != null) {
            ctx.status(200).json(todo);
        } else {
            ctx.status(404).json(new ErrorMessage("Todo not found"));
        }
    }
